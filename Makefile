...
build-all: build test
	...
	env GOOS=linux   GOARCH=arm64         GOWORK=off go build -trimpath -o dist/arm/$(DIST)          ./cmd/...
	env GOOS=linux   GOARCH=arm   GOARM=7 GOWORK=off go build -trimpath -o dist/arm7/$(DIST)         ./cmd/...
	env GOOS=darwin  GOARCH=amd64         GOWORK=off go build -trimpath -o dist/darwin-x64/$(DIST)   ./cmd/...
	env GOOS=darwin  GOARCH=arm64         GOWORK=off go build -trimpath -o dist/darwin-arm64/$(DIST) ./cmd/...
